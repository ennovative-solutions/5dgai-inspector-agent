...
A2A_RPC_URL = BASE_URL + "/a2a/inspector_agent_app/"
...
def start_server() -> subprocess.Popen[str]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "inspector_agent_app.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/inspector_agent_app/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
